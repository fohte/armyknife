...
`--engine codex` launches `codex` instead of `claude` in the no-worktree path (pane command and window-name placeholder); `--prompt` and `--model` are passed to it as `codex --model <model> "<prompt>"`. Combining `--engine` with `--worktree` is rejected: that path always launches `config.wm.layout`'s configured commands, which are independent of `--engine`. A session's engine is recorded on first hook event (see `--engine` on `a agent hook` below) and later read back by `a agent resume` to decide which binary to relaunch -- `resume` always follows that recorded engine, never `agent.default_engine`, so changing the default doesn't affect resuming existing sessions.
