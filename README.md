...
| `--model <model>`                                    | Model for the new session, passed through to `--model` of the `--engine` CLI; accepts whatever that CLI does (e.g. an alias `opus`/`sonnet` or a full name `claude-fable-5` for `claude`, `gpt-5` for `codex`)                                                                                                                                                                                                           |
...
| `--reasoning-effort <low\|medium\|high\|xhigh\|max>` | Codex reasoning effort for this launch only, passed as `codex -c model_reasoning_effort=<effort>` (default: `agent.codex.reasoning_effort` config). Rejected with `--worktree` or when the engine is not `codex`                                                                                                                                                                                                         |
...
`--engine codex` launches `codex` instead of `claude` in the no-worktree path (pane command and window-name placeholder); `--prompt` and `--model` are passed to it as `codex --model <model> -c model_reasoning_effort=<effort> "<prompt>"`. Combining `--engine` with `--worktree` is rejected: that path always launches `config.wm.layout`'s configured commands, which are independent of `--engine`. A session's engine is recorded on first hook event (see `--engine` on `a agent hook` below) and later read back by `a agent resume` to decide which binary to relaunch -- `resume` always follows that recorded engine, never `agent.default_engine`, so changing the default doesn't affect resuming existing sessions.
