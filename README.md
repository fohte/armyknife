...
agent:
  ...
  codex: # defaults for `a agent new --engine codex` only; the `codex` you run yourself is unaffected
    ...
    reasoning_effort: max # low | medium | high | xhigh | max; sent with the first turn when `--reasoning-effort` is omitted
...
| `--reasoning-effort <low\|medium\|high\|xhigh\|max>` | Reasoning effort for this launch only, passed as `claude --effort <effort>` or in the first Codex `turn/start` request (for `codex`, default: `agent.codex.reasoning_effort` config)                                                                                                                                                                                                                                     |
...
`--engine codex` launches `codex` instead of `claude` (pane command and, without `--worktree`, window-name placeholder). When `--prompt` is set, armyknife connects to the shared Codex app-server before opening the pane, launches `codex [--model <model>]` without config overrides, then sends the prompt and reasoning effort in the first `turn/start` request. If the daemon path fails, the same pane is restarted with the prompt in argv and the selected effort as `-c model_reasoning_effort=<effort>`; the command output identifies which route was used. With `--worktree`, the session runs in `config.wm.layout`, whose pane commands are yours to write: every pane running `claude` (e.g. `command: claude`) is replaced by plain `codex`, dropping its arguments because they are Claude Code flags. The layout is left as written when it already has a `codex` pane, and panes running anything else are never touched. A session's engine is recorded on first hook event (see `--engine` on `a agent hook` below) and later read back by `a agent resume` to decide which binary to relaunch -- `resume` always follows that recorded engine, never `agent.default_engine`, so changing the default doesn't affect resuming existing sessions.
