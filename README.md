...
| `--engine <claude\|codex>` | Coding agent CLI to launch (default: `claude`). Only affects the no-worktree path -- see below                                                                                                                                                                                                                                                                                                                           |
...
`--engine codex` launches `codex` instead of `claude` in the no-worktree path (pane command and window-name placeholder). The `--worktree` path always launches `config.wm.layout`'s configured commands regardless of `--engine`. A session's engine is recorded on first hook event (see `--engine` on `a agent hook` below) and later read back by `a agent resume` to decide which binary to relaunch.
...
`a agent hook <event> --engine <claude|codex>` (default: `claude`) records which coding agent CLI fired the hook; this is the engine `a agent resume` reads back to pick which binary to relaunch (see `--engine` on `new` above). Event names and JSON payload shape are the same regardless of engine: `stop` resolves to `Stopped` ("waiting for the next prompt") and `notification` with `notification_type: "permission_prompt"` (or `permission-request`) resolves to `WaitingInput` ("waiting for tool-call approval"), so a Codex `hooks.json` registration that maps Codex's own hook events onto these same `--event` values gets the same approval-wait/input-wait distinction in `a agent list` for free. Wiring that registration is a separate, dotfiles-side change; this repo only accepts and records the `--engine` flag.
