...
Without `--worktree`, `a cc new` compares the target repo (from `-R`, or the current directory) against the repo of the invoking Claude Code session. When they match, it requires running inside an existing tmux pane (it errors otherwise) and splits that pane into a new pane in the same window. When they differ, it instead opens a new tmux window in the target repo's own tmux session.
