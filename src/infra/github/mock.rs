...
impl<'a> MockIssueBuilder<'a> {
    ...
    /// Set the lastEditedAt timestamp.
    pub fn last_edited_at(mut self, ts: &'a str) -> Self {
        self.last_edited_at = Some(ts);
        self
    }

    /// Mount mock for GraphQL issue query (success).
    /// This replaces the REST API mock since get_issue now uses GraphQL.
    /// Uses `body_string_contains("lastEditedAt")` to distinguish from comments query.
    pub async fn get(self) {
        ...
        Mock::given(method("POST"))
            .and(path("/graphql"))
            .and(body_string_contains("lastEditedAt"))
            .respond_with(ResponseTemplate::new(200).set_body_json(json!({
                "data": {
                    "repository": {
                        "issue": {
