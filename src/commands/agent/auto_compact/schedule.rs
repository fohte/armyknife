//! `a agent auto-compact schedule` — the detached worker spawned by the Stop
//! hook.
//!
//! Lifecycle:
//! 1. Stop hook fires `spawn_in_background(session_id)`, which forks a fresh
//!    `a agent auto-compact schedule --session <id>` process and returns
...
/// Spawns a detached `a agent auto-compact schedule --session <id>` so the
/// Stop hook can return immediately. Errors are swallowed (logged to
/// stderr) — failing the hook for an opportunistic optimization is the
/// wrong trade.
...
fn wake_input_for(session: &Session) -> Option<String> {
    ...
    pane::input::get_pane_input_text(pane_id)
}
...
fn spawn_compact_resume(session: &Session) -> Result<()> {
    ...
    // top of `agent hook run`, so the compaction itself doesn't recursively
