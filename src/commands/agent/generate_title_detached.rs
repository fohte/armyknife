//! `a agent generate-title-detached` (hidden) subcommand.
//!
//! Non-interactive title generation designed to be spawned in the
//! background by `a agent watch` (see `tui::title_generate::spawn_detached_title_generation`).
...
use crate::commands::agent::{store, window_status};
use crate::commands::name_branch::{Backend, detect_backend};

/// Tracing target for events emitted by this subcommand.
const EVENT_TARGET: &str = "armyknife::commands::agent::generate_title";
