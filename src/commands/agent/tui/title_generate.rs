...
//!
//! Generation itself never runs inside `agent watch`: pressing Ctrl+g builds
//! the prompt and returns to `AppMode::Normal` immediately, and the actual
//! LLM call happens in a fully detached `a agent generate-title-detached`
...
/// Spawns `a agent generate-title-detached` as a fully detached process,
/// mirroring `clean_progress::spawn_detached_clean` (own session via
/// `setsid`, cwd `/`, stdio to `/dev/null`) so title generation survives
/// `agent watch` exiting. The prompt is written to a persisted temp file
