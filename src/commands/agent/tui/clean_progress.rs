//! Detached `a agent clean-detached` spawn + tracing-log tailing.
//!
//! The clean view spawns the cleanup as a fully detached process (its own
//! session, stdio redirected to `/dev/null`) so that closing `agent watch`
...
use crate::commands::agent::clean_detached;
...
/// Spawn `a agent clean-detached` as a fully detached process. The child
/// gets its own session (so `agent watch` exiting cannot send it SIGHUP),
