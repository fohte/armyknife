...
fn process_hook_event_impl(
    ...
    if event == HookEvent::SessionStart {
        ...
        // (e.g., `a agent new`) can automatically discover the parent session ID.
    ...
    // Stopped, and then SessionEnd would see that status instead of Paused.
    ...
    // of Paused so the TUI shows the session as active and `a agent sweep` can
    ...
    // a concurrent `a agent watch` reader, or self-deadlock against the read,
    ...
    // Notification events.  Notification(permission_prompt) is a status-only
    // event for the same permission ask; clearing the group there would erase
    // the permission notification before the user sees it.
...
/// Determines the event's session status.
...
fn determine_status(event: HookEvent, input: &HookInput) -> SessionStatus {
    match event {
        ...
        // forever, invisible to `a agent sweep`'s idle-timeout check.
...
/// `PermissionRequest` remains notifiable because it carries the tool details.
/// Both engines route it through a delayed worker because another handler can
/// resolve the request before the engine shows the user approval UI.
///
...
/// session until the process ends -- consistent with how `display_status`,
...
fn build_notification_with_message(
    ...
    // Add click action to focus tmux pane via `a agent focus` + app focus
...
mod tests {
    ...
    fn test_build_notification_stop_event_with_pending_bg_task_does_not_claim_stopped() {
        // `session.status` (not `event`) drives the body text: a session
        // whose status is `Running` must not get the "Session stopped"
        // fallback, even on a `Stop` event, and there's no last_message to
        // fall back on either.
        let input = create_test_input(None);
        let mut session = create_test_session(None);
    ...
    fn test_build_notification_with_tmux_info() {
        ...
        assert!(action.command().contains("a agent focus"));
    ...
    #[case::deny_by_hook(
        ...
        serde_json::json!({
            "stop_hook_active": false,
            "last_assistant_message": "permission denied",
        }),
        SessionStatus::Stopped,
    )]
    fn codex_permission_resolution_clears_waiting_state(
        #[case] resolution_event: HookEvent,
        #[case] resolution_event_name: &str,
        #[case] resolution_extra: serde_json::Value,
        #[case] expected_status: SessionStatus,
    ) {
        let temp_dir = tempfile::TempDir::new().expect("temp dir");
        let sessions_dir = temp_dir.path();

        let mut permission_input = parse_stdin_json(&codex_payload(
            "PermissionRequest",
            serde_json::json!({
                "tool_name": "Bash",
                "tool_input": {"command": "echo done"},
        ...
        .expect("Codex permission payload should parse");
        ...
        let mut resolution_input =
            parse_stdin_json(&codex_payload(resolution_event_name, resolution_extra))
                .expect("Codex resolution payload should parse");
        resolution_input.engine = Engine::Codex;
        process_hook_event_impl(
            resolution_event,
            resolution_input,
            sessions_dir,
        ...
        .expect("resolution hook should succeed");

        let session = store::load_session_from(sessions_dir, "codex-lifecycle")
            .expect("load")
            .expect("session exists");
        assert_eq!(
            (session.status, session.pending_permission_agent_ids),
            (expected_status, BTreeSet::new()),
        );
    }

    #[test]
    fn codex_permission_request_stays_waiting_without_resolution() {
        let temp_dir = tempfile::TempDir::new().expect("temp dir");
        let sessions_dir = temp_dir.path();

        let mut input = parse_stdin_json(&codex_payload(
            "PermissionRequest",
            serde_json::json!({
                "tool_name": "Bash",
    ...
    fn stop_with_pending_bg_task_keeps_paused_session_paused() {
        ...
        // shutdown, which maps to `Stopped` regardless of the still-listed
        // task. `keep_paused` must hold the line and not let a dead
        // session's shutdown noise knock it out of `Paused`, since no
        // further hook will ever arrive to correct it.
        let temp_dir = tempfile::TempDir::new().expect("temp dir");
        let sessions_dir = temp_dir.path();
    ...
    // lands in the field matching its own `type`, not the other's. The
    // session still transitions to Stopped: `status` reflects the main loop
    // going idle, not whether a bg task is pending (see
    // `Session::has_pending_bg_tasks` / `display_status`).
    #[case::mixed_types_split_by_field(
        ...
        &["shell-1"], &["task-1"], SessionStatus::Stopped,
    )]
    ...
    #[test]
    fn notification_idle_prompt_does_not_touch_pending_bg_task_ids() {
        ...
        // refresh the pending sets and must leave what the last `Stop`
        // already persisted untouched.
        let temp_dir = tempfile::TempDir::new().expect("temp dir");
        let sessions_dir = temp_dir.path();
        ...
        let reloaded = store::load_session_from(sessions_dir, "idle-bg-sess")
            ...
            .expect("session exists");
        assert_eq!(
            (reloaded.status, reloaded.pending_bg_task_ids),
            (SessionStatus::Stopped, set_of(&["bg-1"])),
        );
    }
    ...
    // still dropped. The session still transitions to Stopped: `status`
    // reflects the main loop going idle, not agent-y's still-pending task
    // (see `Session::has_pending_bg_tasks` / `display_status`).
