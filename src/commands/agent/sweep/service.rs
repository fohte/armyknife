//! launchd agent lifecycle for `a agent sweep`.
//!
//! On macOS, `a agent sweep install` writes a per-user LaunchAgent plist to
//! `~/Library/LaunchAgents/fohte.armyknife.cc-sweep.plist` that runs
//! `a agent sweep run` on a `StartInterval` of 60 seconds, then bootstraps it
...
/// How often launchd should invoke `a agent sweep run`. Five minutes keeps
...
fn ensure_macos() -> Result<()> {
    if cfg!(target_os = "macos") {
        ...
        bail!("`a agent sweep install/uninstall/status` is only supported on macOS")
    }
}
...
<dict>
    ...
    <array>
        ...
        <string>-lc</string>
        <string>a agent sweep run</string>
    </array>
