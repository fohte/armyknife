...
fn build_delegated_prompt(prompt: &str, ctx: &DelegationContext) -> String {
    ...
    context_lines.push("- Contact the delegator via `a agent peer notify` only for one of these two reasons, never for anything else -- not progress updates, not a completion or PR-ready report, not clarifying questions: (1) a premise in these instructions turns out to be wrong, or (2) you were blocked waiting on something under the delegator's control (another repo's fix, a package publish, a prior PR merge, etc.) and it just cleared. Resolve the delegator's session_id at runtime with `a agent peer parent | jq -r '.[0].session_id // empty'`, then run `a agent peer notify --message <MESSAGE> --from \"$ARMYKNIFE_SESSION_ID\" <SESSION_ID>` (the `--from` names this session so the delegator, which may be juggling several delegates, can tell who sent it; notify also resumes the delegator automatically if it's paused). If the session_id is empty, the delegator isn't tracked by armyknife -- report to the user instead of guessing a target".to_string());
