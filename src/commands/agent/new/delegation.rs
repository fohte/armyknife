...
use crate::commands::agent::store as cc_store;
...
/// (e.g. `a agent new` without `--worktree`), in which case those lines are
...
fn build_delegated_prompt(prompt: &str, ctx: &DelegationContext) -> String {
    ...
    context_lines.push("- Contact the delegator via `a agent peer notify` only in these four cases: (1) a premise in these instructions turns out to be wrong, (2) you were blocked waiting on something under the delegator's control (another repo's fix, a package publish, a prior PR merge, etc.) and it just cleared, (3) you finished work in direct-commit mode, where merged-worktree cleanup cannot provide an automatic completion notification, or (4) you cannot continue before reaching the goal and are about to stop. Never use it for anything else: do not send progress updates or clarifying questions, and do not send completion or PR-ready reports for normal PR-based delegations because armyknife notifies the delegator automatically when the merged worktree is cleaned up. Resolve the delegator's session_id at runtime with `a agent peer parent | jq -r '.[0].session_id // empty'`, then run `a agent peer notify --message <MESSAGE> <SESSION_ID>` (notify identifies this session to the delegator automatically, so it can tell who sent it when juggling several delegates; it also resumes the delegator automatically if it's paused). If the session_id is empty, the delegator isn't tracked by armyknife -- report to the user instead of guessing a target".to_string());
...
mod tests {
    ...
    #[case::single_line(
        ...
        indoc! {"
            ...
            - Contact the delegator via `a agent peer notify` only in these four cases: (1) a premise in these instructions turns out to be wrong, (2) you were blocked waiting on something under the delegator's control (another repo's fix, a package publish, a prior PR merge, etc.) and it just cleared, (3) you finished work in direct-commit mode, where merged-worktree cleanup cannot provide an automatic completion notification, or (4) you cannot continue before reaching the goal and are about to stop. Never use it for anything else: do not send progress updates or clarifying questions, and do not send completion or PR-ready reports for normal PR-based delegations because armyknife notifies the delegator automatically when the merged worktree is cleaned up. Resolve the delegator's session_id at runtime with `a agent peer parent | jq -r '.[0].session_id // empty'`, then run `a agent peer notify --message <MESSAGE> <SESSION_ID>` (notify identifies this session to the delegator automatically, so it can tell who sent it when juggling several delegates; it also resumes the delegator automatically if it's paused). If the session_id is empty, the delegator isn't tracked by armyknife -- report to the user instead of guessing a target
            </context>
            <instructions>
            Fix the auth bug
            </instructions>
