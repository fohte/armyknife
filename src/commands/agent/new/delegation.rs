...
fn build_delegated_prompt(prompt: &str, ctx: &DelegationContext) -> String {
    ...
    context_lines.push("- Contact the delegator via SendMessage only for one of these two reasons, never for anything else -- not progress updates, not a completion or PR-ready report, not clarifying questions: (1) a premise in these instructions turns out to be wrong, or (2) you were blocked waiting on something under the delegator's control (another repo's fix, a package publish, a prior PR merge, etc.) and it just cleared. Resolve the delegator's name at runtime with `a agent peer parent | jq -r '.[0].name // empty'`. If that is empty, the delegator may be paused -- run `a agent peer wake $(a agent peer parent | jq -r '.[0].session_id')` to resume it and get back a usable name".to_string());
...
mod tests {
    ...
    #[case::single_line(
        ...
        indoc! {"
            ...
            - Contact the delegator via SendMessage only for one of these two reasons, never for anything else -- not progress updates, not a completion or PR-ready report, not clarifying questions: (1) a premise in these instructions turns out to be wrong, or (2) you were blocked waiting on something under the delegator's control (another repo's fix, a package publish, a prior PR merge, etc.) and it just cleared. Resolve the delegator's name at runtime with `a agent peer parent | jq -r '.[0].name // empty'`. If that is empty, the delegator may be paused -- run `a agent peer wake $(a agent peer parent | jq -r '.[0].session_id')` to resume it and get back a usable name
            </context>
            <instructions>
            Fix the auth bug
            </instructions>
