...
/// a polling-based `window-status-format` that calls `#(a agent window-status)`.
...
/// This is what turns the per-redraw polling of `#(a agent window-status)` into
/// an event-driven update fired only by `a agent hook`.
