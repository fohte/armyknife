...
fn find_by_session_option(session_id: &str) -> Option<PaneInfo> {
    ...
    let active_codex_panes = panes
        .into_iter()
        .filter(|pane| pane.option_value.as_deref() == Some(session_id))
        .filter(|pane| {
            tmux::get_pane_current_command(&pane.pane_id).as_deref()
                == Some(Engine::Codex.process_name())
        })
        .collect::<Vec<_>>();
    let pane_id = unique_pane_for_session(&active_codex_panes, session_id)?;

    tmux::get_pane_info_by_pane_id(pane_id)
}

fn unique_pane_for_session<'a>(
