...
fn build_delegated_prompt(prompt: &str, ctx: &DelegationContext) -> String {
    indoc::formatdoc! {"
        ...
        - If a premise in these instructions turns out to be wrong, report it to the delegator via SendMessage; resolve its name at runtime with `a cc peer parent | jq -r '.[0].name // empty'`. If that is empty, the delegator may be paused -- run `a cc wake $(a cc peer parent | jq -r '.[0].session_id')` to resume it and get back a usable name (do not use it to ask questions or send progress updates)
        </context>
        <instructions>
        {prompt}
        </instructions>
