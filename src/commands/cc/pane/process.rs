...
/// any descendant -- currently has a running `claude` process. Used to
...
/// "has claude", not "no claude", since the risk this guards against is
/// retyping into a live conversation, not skipping a resume that turns out
/// to be safe.
pub fn pane_has_live_claude_process(pane_pid: u32, snapshot: Option<&ProcessSnapshot>) -> bool {
